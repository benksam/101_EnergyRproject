...
data_Crisis<- read.csv("KL_Crisis_PCA.csv",header=TRUE,sep=",", dec=".", row.names=1)
str(data_Crisis)
...
which(rownames(data_Crisis)=="999")

res <- PCA(data_Crisis[1:96,], scale.unit=TRUE,graph = F,
...
Crisis_scale = scale(data_Crisis[1:95,3:16])
head(Crisis_scale)

write.csv(Crisis_scale,"KL_Crisis_PCA_Scaled.csv")
...
estim_ncp(data_Crisis[1:95,3:16], scale = TRUE, method = "Smooth")     # suggestion de NCP
?estim_ncp
...
dimdesc(res)
dimdesc(res, axes = 4:5)

?dimdesc
?catdes

catdes(data_Crisis[1:95,],num.var=1) # Description de la var. categorielle par autres var.**

condes(data_Crisis[1:95,], num.var=3) # Description de la var. continue par autres variables (cor/eta2)
...
x11()
pairs(data_Crisis[1:95,3:16], upper.panel = panel.cor,
      panel=function(x,y){
        points(x,y)
...
corm <- cor(data_Crisis[1:95,3:16])
class(corm)
...
# # Comparaison et pourcentage de correspondance, MAIS NON INDEXEE, ininterCrisistable!
# par(mfcol=c(2,3))
# 
# for (i in 2:4) {
#   plot(res$ind$coord[,1],res$ind$coord[,2],
